...
wemascreener.py
================
Weekly EMA alignment screener for NSE stocks (e.g. Nifty 500).

Logic
-----
1. Download weekly OHLCV data for every ticker in the universe.
2. Compute 10-week, 30-week and 40-week EMAs.
3. ALIGNMENT condition: EMA10 > EMA30 > EMA40 (stacked bullish order).
4. PROXIMITY condition: the current weekly close is within 7% of *each*
   of the three EMAs (price hugging the moving-average cluster rather
   than extended away from it).
...
def evaluate_ticker(ticker: str, df: pd.DataFrame) -> dict | None:
    ...
    Conditions
    ----------
    1. EMA alignment (stacked uptrend order): EMA10 > EMA30 > EMA40.
    2. Proximity: the current weekly close is within EMA_PROXIMITY_BAND
       (7%) of *each* of the three EMAs — i.e. price is hugging the moving
       average cluster rather than extended away from it.
    3. Liquidity: average weekly volume over the lookback window is above
    ...
    try:
        ...
        if is_aligned and is_close_to_emas and is_liquid:
...
def write_outputs(df: pd.DataFrame):
    ...
    lines = [
        ...
        "",
        f"**{len(df)} stocks matched**: EMA10 > EMA30 > EMA40 (stacked uptrend), "
        f"weekly close within {int(EMA_PROXIMITY_BAND * 100)}% of all three EMAs, "
...
def main():
    parser = argparse.ArgumentParser(description="Weekly EMA alignment screener")
