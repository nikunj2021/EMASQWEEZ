...
wemascreener.py
================
Weekly EMA proximity screener for NSE stocks (e.g. Nifty 500).
...
def evaluate_ticker(ticker: str, df: pd.DataFrame) -> dict | None:
    ...
    try:
        ...
        if is_above_all and is_close_to_emas and is_liquid:
...
def write_outputs(df: pd.DataFrame):
    ...
    lines = [
        ...
        f"within {int(EMA_PROXIMITY_BAND * 100)}% of all three EMAs, "
...
def main():
    parser = argparse.ArgumentParser(description="Weekly EMA proximity screener")
